...
Если нужно посчитать среднюю сумму заказов для каждого города, использовать фильтры уже неудобно — придётся  
написать много запросов. А перед этим нужно получить список уникальных городов, например, с помощью DISTINCT.  
Для таких задач подойдёт группировка.
...
**В запросе GROUP BY всегда располагается после предложения WHERE!**  

Группировка по нескольким полям:  
общее количество продаж на каждый день, для каждой страны
```
SELECT 
    invoice_date::date,
    billing_country,
    COUNT(total)
FROM invoice
GROUP BY invoice_date::date, billing_country 
...
В остальном две таблицы похожи, потому что оператор GROUP BY собрал группы из всех уникальных комбинаций в полях invoice_date и billing_country.
...
Например, в этом запросе столбец billing_country указан в SELECT, но не указан в GROUP BY, Поэтому запрос приведёт к ошибке:    
